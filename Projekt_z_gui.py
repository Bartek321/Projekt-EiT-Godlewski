...
def guio():                         #mini gui do dodawania slow kluczowych i pauzy
    def press(button):
        global Pause
        if button == "Dodaj":
            a = app.getEntry("Klucz")
            b = app.getEntry("Slowo")
        ...
        if button == "Usun":
            ...
            Pause = not Pause
            if Pause == True:
                app.setButton("Pauza", "Start")
            else:
                app.setButton("Pauza", "Pauza")
            

    app = gui()

    app.addLabelEntry("Klucz")
    app.addLabelEntry("Slowo")
...
if __name__ == '__main__':
    ...
    t.start()
    z = threading.Thread(target=guio, args=())        #start watku
    z.start()
